...
funkcja funkcja_trasy(Point, Point);
...
uint16_t t_180_stopni = 3600;
uint16_t t_90_stopni = 1800;
uint16_t t_1_stopni = 20;
float obecny_obrot = 0;


const Point punkt_start = {0,0};
const Point punkt_meta = {500, 1000};
Point pozycja = punkt_start;
uint16_t kat_startu;
...
funkcja funkcja_trasy(Point p1, Point p2){
    funkcja f;
    if(p1.y > p2.y){
        Point temp = p2;
        p2 = p1;
...
void app_main()
{
    ...
    gpio_config(&IO_config);
		...
		RTC_reset();
        funkcja trasa = funkcja_trasy(punkt_start, punkt_meta);
        obecny_obrot = atan2(trasa.a, 1)*180.0/M_PI;
        sterowanie_silnikami("lewo",50);
        vTaskDelay(obecny_obrot*t_1_stopni/portTICK_PERIOD_MS);
        sterowanie_silnikami("stop",69); 
        ...
        {	
            ...
            // todo: dodac funkcje sprawdzajaca czy wrocilismy na trase i array z trasa
            if(!omijanie){
                sterowanie_silnikami("przod", 50);
            } else {
                if(poczatek_omijania){
                    uint64_t czas = esp_timer_get_time();
                    while(distance_cm[4]>7){ // musimy stestowac jaka wartosc bedzie oznaczala ze pojazd jest rownolegle do przeszkody
                        sterowanie_silnikami("lewo", 50);
                    }
                    czas = esp_timer_get_time() - czas; // do stestowania ile trwa skret
                    uint16_t kat_startu = czas*1000/t_1_stopni;
                    obecny_obrot = obecny_obrot + kat_startu;
                    poczatek_omijania = false;
                }
                sterowanie_silnikami("przod", 50);
                if(distance_cm[4]<7){
                    sterowanie_silnikami("prawo", 50);
                    vTaskDelay(t_90_stopni / portTICK_PERIOD_MS); // do stestowania ile trwa skret, tu musi byc obrot o kat 90 stopni
                    parzysta_liczba_skretow = !parzysta_liczba_skretow;
                }
                for(uint8_t i =1; i<3;i++){
                    if(distance_cm[i]<=10){
                        sterowanie_silnikami("lewo", 50);
                ...
                if((pozycja.y-1 < ((trasa.a * pozycja.x) + trasa.b)) && (pozycja.y+1 > ((trasa.a * pozycja.x) + trasa.b))){
                    sterowanie_silnikami("lewo", 50);
                    uint16_t kat_powrotu = (parzysta_liczba_skretow) ? (kat_startu - 90):(180 - kat_startu);
                    uint16_t czas_powrotu = kat_powrotu/t_1_stopni;
                    obecny_obrot = obecny_obrot + kat_powrotu;
                    sterowanie_silnikami("lewo", 50);
                    vTaskDelay(czas_powrotu / portTICK_PERIOD_MS);

                    omijanie = false;
                    poczatek_omijania = true;
                    parzysta_liczba_skretow = true;
                }
                obecny_obrot = (obecny_obrot>=360) ? (obecny_obrot - 360):obecny_obrot;
                obecny_obrot = (obecny_obrot<0) ? (360+obecny_obrot):obecny_obrot;

            }
            pozycja.x = (travelled_distance - prev_distance)*cos(obecny_obrot*M_PI/180.0);
            pozycja.y = (travelled_distance - prev_distance)*sin(obecny_obrot*M_PI/180.0);
            prev_distance = travelled_distance;
