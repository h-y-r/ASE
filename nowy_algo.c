...
#define ODB_KAT_ADC ADC1_CHANNEL_9             // gpio dla czujnika odbiciowego katowego
#define HALL_ADC ADC1_CHANNEL_7                    // gpio dla czujnika halla
...
Point start_omijania;

double funkcja_trasy(double);
bool start = true;
...
uint16_t kat_powrotu;
uint8_t tick_powrotu;
int szczel_state_interrupt;

bool omijanie = false;
bool omijanie_prawo = false;
bool poczatek_omijania = true;
bool parzysta_liczba_skretow = true;
uint8_t turn90 = 19;
uint8_t turn180 = 38;
uint8_t turn4_75 = 1; 
signed int tickCount = 0;
float obecny_obrot = 0;
bool countReset = true;
bool override = false;
uint8_t akcja = 12;
uint8_t tick_start;
...
double kat_do_mety = 50;
Point pozycja = {0.0,0.0};
uint16_t kat_startu;
...
double funkcja_trasy(double angle){
...
static void IO_Handler(void *Data_struct) {
    ...
    else
    {
            ...
            // if(((Ultrasonic_data_t*)Data_struct)->Current_sensor == 0)
            // {
            //     if(((Ultrasonic_data_t*)Data_struct)->Distance[((Ultrasonic_data_t*)Data_struct)->Current_sensor]/58.0 >= 50)
            //     {
            //         Wolna_lewa++;
            //     }
            //     else
...
void mcpwm_init_mostek_h()
{
    ...
    mcpwm_set_signal_low(MCPWM_UNIT_0, MCPWM_UNIT_0, MCPWM_OPR_A);
    mcpwm_set_signal_high(MCPWM_UNIT_0, MCPWM_UNIT_0, MCPWM_OPR_B);
    mcpwm_set_signal_low(MCPWM_UNIT_0, MCPWM_UNIT_1, MCPWM_OPR_A);
    mcpwm_set_signal_high(MCPWM_UNIT_0, MCPWM_UNIT_1, MCPWM_OPR_B);

}

void sterowanie_silnikami(const char* kierunek)
{
    if (strcmp(kierunek, "prawo") == 0) { // dziala więc nie będę się czepiać
        ...
        mcpwm_set_duty(MCPWM_UNIT_0, MCPWM_TIMER_1, MCPWM_OPR_B, 75);
        mcpwm_set_signal_low(MCPWM_UNIT_0, MCPWM_UNIT_1, MCPWM_OPR_A);
    ...
    } else if (strcmp(kierunek, "przod") == 0) {
		mcpwm_set_duty_type(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM_OPR_B, MCPWM_DUTY_MODE_1);
        mcpwm_set_duty(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM_OPR_B, 55);		//lewy
        mcpwm_set_signal_low(MCPWM_UNIT_0, MCPWM_UNIT_0, MCPWM_OPR_A);
		mcpwm_set_duty_type(MCPWM_UNIT_0, MCPWM_TIMER_1, MCPWM_OPR_A, MCPWM_DUTY_MODE_0);
        mcpwm_set_duty(MCPWM_UNIT_0, MCPWM_TIMER_1, MCPWM_OPR_A, 68);		//prawy
        mcpwm_set_signal_high(MCPWM_UNIT_0, MCPWM_UNIT_1, MCPWM_OPR_B);
        
    } else if (strcmp(kierunek, "tyl") == 0) {
		mcpwm_set_duty_type(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM_OPR_A, MCPWM_DUTY_MODE_0);
        mcpwm_set_duty(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM_OPR_A, 85);
        mcpwm_set_signal_high(MCPWM_UNIT_0, MCPWM_UNIT_0, MCPWM_OPR_B);
		mcpwm_set_duty_type(MCPWM_UNIT_0, MCPWM_TIMER_1, MCPWM_OPR_B, MCPWM_DUTY_MODE_1);
        mcpwm_set_duty(MCPWM_UNIT_0, MCPWM_TIMER_1, MCPWM_OPR_B, 67);
        mcpwm_set_signal_low(MCPWM_UNIT_0, MCPWM_UNIT_1, MCPWM_OPR_A);
...
void app_main()
{
    ...
    adc_oneshot_config_channel(adc1_handle, HALL_ADC, &adc_chan_cfg); //hall
    adc_oneshot_config_channel(adc1_handle, ODB_KAT_ADC, &adc_chan_cfg); //kat
    ...
    gpio_config(&IO_config);
	    ...
	    // ustawienie reakcji na dane zbocze (aktualnie - zbocze opadające - inkrementacja i zbocze narastające - nic)
...
//		}
//		RTC_reset();
        double trasa = funkcja_trasy(kat_do_mety);
        while(start_driving == 0);

        bool przejscie = false;
        signed int tickCount2;
        signed int start_przod;
        signed int stop_przod;
        Point poprzednia_pozycja;
			
        while(1)
        {	
            bool odb_kat_state = false;
            int odb_kat_adc_reading = 0;
            adc_oneshot_read(adc1_handle, ODB_KAT_ADC, &odb_kat_adc_reading);
            if(odb_kat_adc_reading < 90)
            {
                odb_kat_state = 1;
            }
            else
            ...
            adc_oneshot_read(adc1_handle, HALL_ADC, &hall_adc_reading);
            bool hall_state = 0;
            ...
            if(start){
                    tick_start = kat_do_mety/4.5;
                    obecny_obrot = kat_do_mety;
                    pcnt_unit_get_count(pcnt_unit, &tickCount);
                    pcnt_unit_get_count(pcnt_unit, &tickCount2);
                    while((tickCount2 - tickCount) < tick_start) {
                        sterowanie_silnikami("lewo");
                        pcnt_unit_get_count(pcnt_unit, &tickCount2);
                    }
                    sterowanie_silnikami("stop");
            ...
            switch(akcja){
                case 0:
                    sterowanie_silnikami("przod");
                    pcnt_unit_get_count(pcnt_unit, &start_przod);
                    if(Ultrasonic_data.Distance[0]/58 >= 50){
                        sterowanie_silnikami("stop");
                        pcnt_unit_get_count(pcnt_unit, &stop_przod);
                        pozycja.x = pozycja.x + (stop_przod - start_przod)*cos(obecny_obrot*M_PI/180.0);   
                        pozycja.y = pozycja.y + (stop_przod - start_przod)*sin(obecny_obrot*M_PI/180.0);                
                        tick_start = 20;
                        pcnt_unit_get_count(pcnt_unit, &tickCount);
                        pcnt_unit_get_count(pcnt_unit, &tickCount2);
                        while((tickCount2 - tickCount) < tick_start) {
                        ...
                        //akcja = 1;
                        obecny_obrot = obecny_obrot + 90;
                    }
                    for(int i = 1; i<4; i++){
                        if(Ultrasonic_data.Distance[i]/58 <= 10){
                            sterowanie_silnikami("stop");
                            pcnt_unit_get_count(pcnt_unit, &stop_przod);
                            pozycja.x = pozycja.x + (stop_przod - start_przod)*cos(obecny_obrot*M_PI/180.0);   
                            pozycja.y = pozycja.y + (stop_przod - start_przod)*sin(obecny_obrot*M_PI/180.0);   
                    ...
                    if(pozycja.y-5<(trasa*pozycja.x) && pozycja.y+5>(trasa*pozycja.x) && (sqrt(pozycja.x*pozycja.x + pozycja.y*pozycja.y)-sqrt(poprzednia_pozycja.x*poprzednia_pozycja.x + poprzednia_pozycja.y*poprzednia_pozycja.y))>5){
                        if(obecny_obrot > kat_do_mety){
                            tick_start = (obecny_obrot - kat_do_mety)/4.5;
                            pcnt_unit_get_count(pcnt_unit, &tickCount);
                            pcnt_unit_get_count(pcnt_unit, &tickCount2);
                            while((tickCount2 - tickCount) < tick_start) {
                                sterowanie_silnikami("prawo");
                            ...
                            sterowanie_silnikami("stop");
                        } else if (obecny_obrot < kat_do_mety) {
                            tick_start = (kat_do_mety - obecny_obrot)/4.5;
                            pcnt_unit_get_count(pcnt_unit, &tickCount);
                            pcnt_unit_get_count(pcnt_unit, &tickCount2);
                            while((tickCount2 - tickCount) < tick_start) {
                                sterowanie_silnikami("lewo");
                ...
                case 12:
                    ...
                    while (!przejscie){
                        ...
                        for(int i = 1; i<4; i++){
                            if(Ultrasonic_data.Distance[i]/58.0 <= 10.0){
                                sterowanie_silnikami("stop");
                                pcnt_unit_get_count(pcnt_unit, &stop_przod);
                                pozycja.x = pozycja.x + (stop_przod - start_przod)*cos(obecny_obrot*M_PI/180.0);   
                                pozycja.y = pozycja.y + (stop_przod - start_przod)*sin(obecny_obrot*M_PI/180.0);
                    ...
                    //         sterowanie_silnikami("stop");
                    //         tickCount = szczel_state;
                    //         countReset = false;
                    //     }
                    //     sterowanie_silnikami("prawo");
                    //     if(szczel_state >= tickCount + tick_start){
                    //         sterowanie_silnikami("stop");
                    //         countReset = true;
                    //         przejscie = false;
                    //         obecny_obrot = obecny_obrot - 90;
            ...
            obecny_obrot = (obecny_obrot>=360) ? (obecny_obrot - 360):obecny_obrot;
            obecny_obrot = (obecny_obrot<0) ? (360+obecny_obrot):obecny_obrot;
            // test silników
//            sterowanie_silnikami("przod");
